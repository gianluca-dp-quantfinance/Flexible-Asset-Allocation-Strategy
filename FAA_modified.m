...
strategies_list = {
   ...
   % RAVC 4-4-4
    struct('name','RAVC_enhanced 4-4-4', ...
           'lookback_momentum',4, ...
           'lookback_volatility',4, ...
    ...
    % RAVC 4-3-3
    struct('name','RAVC_enhanced 4-3-3', ...
           'lookback_momentum',4, ...
    ...
    % RAVC 5-5-5
    struct('name','RAVC_enhanced 5-5-5', ...
           'lookback_momentum',5, ...
    ...
    % RAVC 3-3-3
    struct('name','RAVC_enhanced 3-3-3', ...
           'lookback_momentum',3, ...
           'lookback_volatility',3, ...
           'lookback_correlation',3, ...
           'w_volatility',0.5, ...
...
for s = 1:numel(strategies_list)
    ...
    [equity_tt, drawdown_tt, weights_tt] = runBacktestStrategy_modified( ...
...
for p = 1:numel(backtest_periods)
    ...
    for s = 1:numel(strategy_names)
        ...
        % Rinormalizza a capitale iniziale
        eq_period{:,1} = 100000 * eq_period{:,1} / eq_period{1,1};

        % Plot
        plot(eq_period.dates_returns, eq_period{:,1}, 'LineWidth', 1.5);
    end
    grid on;
    set(gca,'YScale','log');
    xlabel('Data'); ylabel('Equity (log scale)');
    ...
    for s = 1:numel(strategy_names)
        ...
        % Filtra il sottoperiodo
        eq_period = eq(eq.dates_returns >= period.start_date & ...
                       eq.dates_returns <= period.end_date, :);

        % Rinormalizza
        eq_period{:,1} = 100000 * eq_period{:,1} / eq_period{1,1};

        % Calcolo drawdown
        peak_eq = cummax(eq_period{:,1});
        dd_values = (eq_period{:,1} - peak_eq) ./ peak_eq * 100;
...
for p = 1:numel(backtest_periods)
    ...
    results_summary.(period_field) = table( ...
        ...
        'VariableNames', {'CAGR','Vol','Sharpe','Sortino','SortinoMod','Calmar','Validity','Omega', ...
                          'MaxDD','VaR5%','ES5%','BestM','WorstM','ProfM', ...
...
writetable(annual_return_table, 'Annual_Returns_Per_StrategyModified.csv', 'WriteRowNames', true);
